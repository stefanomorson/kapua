...
public class AccessInfosResource extends AbstractKapuaResource {
        ...
        new AuthenticatedSessionExecutor(vertx, headers).exec(
                future -> {
                    try {
                        ...
                        AccessInfoListResult result = accessInfoService.query(query);
    ...
    public void count(
        ...
        new AuthenticatedSessionExecutor(vertx, headers).exec(
                future -> {
                    try {
                        query.setScopeId(scopeId);
                        CountResult result = new CountResult(accessInfoService.count(query));
                        future.complete(result);
                    } catch (KapuaException exc) {
                        future.fail(exc);
                    }
        ...
        new AuthenticatedSessionExecutor(vertx, headers).exec(
                future -> {
                    try {
                        ...
                        AccessInfo result = accessInfoService.create(accessInfoCreator);
                        future.complete(result);
                    } catch (KapuaException exc) {
                        future.fail(exc);
                    }
